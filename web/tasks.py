...
def create_waveform(waveform):
    ...
    track_tmp_file = settings.MEDIA_ROOT + '/track_tmp/%d%s' % (waveform.id, track_ext[1])
    track_tmp_wavfile = settings.MEDIA_ROOT + '/track_tmp/%d.wav' % (waveform.id)
    ...
    waveform_img = settings.MEDIA_ROOT + '/tmp/%s_w.png' % str(waveform.id)
    spectrum_img = settings.MEDIA_ROOT + '/tmp/%s_s.jpg' % str(waveform.id)
